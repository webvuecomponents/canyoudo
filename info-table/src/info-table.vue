<template>
  <div>
    <el-row>
      <el-col :span='!val.col ? 24 / num : 24 / num * val.col' v-for="(val, ind) in tableData" :key='ind'>
      <div class='wrapperS'
        :style="wrapperStyle"
        :class="{
        ...
        <div class='colName' :style='handleStyle(val.style, nameStyle)' :title="val.name">
          <span v-if='val.required' class='required'></span>{{val.name}}
        </div>
        <div class='colValue' :style='handleStyle(val.style, valueStyle)' :title="val.value">
          <slot :name='val.key || "default"'>
            <div  v-html='val.html || val.format&&val.format(val.value || dataValue[val.key], val, dataValue) || val.value || dataValue&&dataValue[val.key]'></div>
          </slot>
        </div>
      </div>
      </el-col>
...
<script>
   ...
   */
export default {
  props: ['dataValue', 'tableData', 'num', 'nameStyle', 'valueStyle', 'wrapperStyle'],
  methods: {
